...
Respect pause and workload guards immediately. Avoid local inference while Android emulators, device tests, graphics builds, rendering, video work, games, or other GPU-heavy workloads are active unless the user explicitly prioritizes local review. Prefer passive health checks or primary Codex capabilities during contention. Entry-tier installations use low-impact mode, one request at a time, conservative limits, and immediate unload by default; keep-warm mode is opt-in.
