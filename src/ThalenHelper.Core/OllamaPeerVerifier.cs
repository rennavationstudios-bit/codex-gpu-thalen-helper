...
internal static class OllamaPeerVerifier
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new OllamaPeerIdentityException("The Ollama connection target was not IPv4 loopback.");
        }
        ...
        try
        {
            ...
            var processId = await FindServerProcessIdAsync(socket, cancellationToken).ConfigureAwait(false)
                ?? throw new OllamaPeerIdentityException("The process that owns the Ollama TCP connection could not be identified.");
            processHandle = OpenProcess(ProcessQueryLimitedInformation | Synchronize, false, processId);
            if (processHandle.IsInvalid)
            {
                throw new OllamaPeerIdentityException("The process that owns the Ollama TCP connection could not be opened safely.");
            }
            ...
            var signatureValid = AuthenticodeVerifier.Verify(executablePath, "Ollama Inc.");
            if (!IsTrustedIdentity(executablePath, processSid, currentSid, signatureValid)
                || WaitForSingleObject(processHandle, 0) != WaitTimeout)
            {
                throw new OllamaPeerIdentityException("The loopback endpoint is not owned by the expected current-user Ollama process.");
            }
        ...
        bool signatureValid)
    {
        ...
        return string.Equals(fileName, "ollama.exe", StringComparison.OrdinalIgnoreCase)
            || string.Equals(fileName, "ollama app.exe", StringComparison.OrdinalIgnoreCase);
    }

    private static async Task<uint?> FindServerProcessIdAsync(Socket socket, CancellationToken cancellationToken)
    {
    ...
    private static uint? FindServerProcessId(Socket socket)
    {
        ...
        var size = 0;
        var result = GetExtendedTcpTable(IntPtr.Zero, ref size, false, AfInet, TcpTableOwnerPidConnections, 0);
        ...
        try
        {
            result = GetExtendedTcpTable(buffer, ref size, false, AfInet, TcpTableOwnerPidConnections, 0);
            if (result != 0)
            {
                return null;
            }
            ...
            var firstRowOffset = Marshal.OffsetOf<MibTcpTableOwnerPidHeader>(nameof(MibTcpTableOwnerPidHeader.FirstRow)).ToInt32();
    ...
    private static string QueryProcessImagePath(SafeProcessHandle processHandle)
    {
        ...
        {
            throw new OllamaPeerIdentityException("The Ollama peer executable path could not be read.");
        }
    ...
    private static string QueryProcessUserSid(SafeProcessHandle processHandle)
    {
        ...
        {
            throw new OllamaPeerIdentityException("The Ollama peer user identity could not be opened.");
        }
        ...
        try
        {
            ...
            if (requiredLength <= 0)
            {
                throw new OllamaPeerIdentityException("The Ollama peer user identity size could not be determined.");
            }

            var buffer = Marshal.AllocHGlobal(requiredLength);
            try
            {
                if (!GetTokenInformation(tokenHandle, TokenUserInformationClass, buffer, requiredLength, out _))
                {
                    throw new OllamaPeerIdentityException("The Ollama peer user identity could not be read.");
                }
    ...
    [return: MarshalAs(UnmanagedType.Bool)]
    private static extern bool OpenProcessToken(SafeProcessHandle processHandle, uint desiredAccess, out IntPtr tokenHandle);
...
internal sealed class OllamaPeerIdentityException(string message) : IOException(message);
