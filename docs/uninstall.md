...
For a product-owned integration, lifecycle cleanup disables/cancels review, observes whether a tracked low-impact generation has released, removes the owned per-user Ollama startup entry, restores prior `OLLAMA_MODELS`/`OLLAMA_HOST` values only if the current values are still product-owned, removes only the marked MCP/instruction sections, deletes product state, and writes a concise report in the temporary directory. It never issues an unload or deletion request by mutable Ollama model name. For LM Studio it can unload only an exact helper-created tracked instance whose ownership is still provable; otherwise it leaves every runtime untouched and reports that release could not be confirmed.

When setup preserved an existing unmarked `local_gpu_reviewer`, uninstall does not connect to Ollama or LM Studio, unload or delete models, change startup/environment values, terminate reviewer processes, or edit the existing TOML table. It removes only helper-managed instruction sections and product state.

Codex authentication, unrelated config/instructions, Ollama, LM Studio, and all model data are preserved. Neither provider is automatically uninstalled. The legacy `--remove-owned-model` switch is accepted for command compatibility but deliberately performs no model deletion because Ollama's delete API targets a mutable name rather than an immutable digest, and LM Studio GGUF files remain user-owned.
...
Ollama model directories and every LM Studio GGUF are preserved. Delete model data manually only after confirming no other application uses it. Uninstalling the helper does not undo, move, or delete the user's LM Studio models-root junction or directory.
